// Note: We recommend that after you complete a challenge, you re-comment out the
// test case for that challenge so the console output is not confusing when working
// on subsequent challenges.

/* CHALLENGE 1 */
/* 
Thinking point (no writing code necessary for this challenge): Inspect the code 
given to you in Challenge 1. In what order should the console logs come out? Howdy 
first or Partnah first? 
*/
function sayHowdy() {
  console.log("Howdy");
}
...
/* CHALLENGE 3 */
// Create a function helloGoodbye that console logs hello right away, and good bye after
// 2 seconds.
...
function limitedRepeat() {
  ...
  setTimeout(clearInterval, 5000, setInterval(callback, 1000));
}
...
everyXsecsForYsecs will execute the given function every interval number of 
milliseconds, but then automatically stop after duration milliseconds.
...
function promised(val) {
  ...
  return new Promise((resolve, reject) => {
    ...
    //   resolve(val);
    // }, 2000);
    setTimeout(clearInterval, 2000, resolve(val));
  });
}
...
on an argument every second, with the argument to the callback being the current 
seconds "value".
...
/* CHALLENGE 10 */
/*
Write a function called debounce that accepts a function and returns a new 
function that only allows invocation of the given function after interval 
milliseconds have passed since the last time the returned function ran.

Additional calls to the returned function within the interval time should not 
be invoked or queued, but the timer should still get reset.

For examples of debouncing, check out 
